...
if($validaciones->validacadenas($nmarcas)=="error")
{
...
if($validaciones->validacadenas($detalle)=="error")
{
...
if($valida=="OK")
{
  ...
  $marcas = new marcas();
  $marcas->setnmarcas($nmarcas);
  $marcas->setdetalle($detalle);
  $marcas->setactivo($activo);
  $marcas->setidpro($idpro);
  $marcas->setarchivo($nombrearchivo);
  $marcas->setruta("ImMar/$nombrearchivo");
  $resultado = $marcas->insertaMarca();

  if ($resultado == 1)
  {
  move_uploaded_file($_FILES['archivo']['tmp_name'],"ImMar/$nombrearchivo");
    echo"<center><H1>Marca registrada</H1></center>";
