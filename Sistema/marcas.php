        ...
        <div class="sidebar content-box" style="display: block;">
                <ul class="nav">
                    ...
                    <li><a href="productos.php"><i class="glyphicon glyphicon-shopping-cart"></i>Productos</a></li>
                       ...
                       <li class="current" class=><a href="marcas.php"><i class="glyphicon glyphicon-list-alt"></i> Marcas</a></li>
...
$queryMar = "SELECT m.idmar, m.nmarcas, m.detalle, m.activo, m.ruta 
...
while($row = $resMar->fetch_assoc()) {
  ...
  $idmar = $row['idcli'];
  $nmarcas = $row['nombrecli'];
  $detalle = $row['appcliente'];
  $activo = $row['sexo'];
  $ruta = $row['emailcli'];
...
if($cuantos==0)
{
  ...
  echo "<center><h2>Aún no tienes ninguna <b>marca</b> regustrado <h2>
<img src ='ImMar/sombramarca.png' height = 100><center></center>";
...
echo "<th>Foto</th><th>Marca</th><th>Descripción</th><th>Activo</th><th>Opciones</th></tr>";




echo "<tr><td><center><center><img src ='$ruta' height = 100><center></td><td>$nmarcas</td><td>$detalle</td><td>$activo</td><td><a href='modificarmarca.php?idmar=$idmar'</a><button class='btn btn-default btn-xs'><i class='glyphicon glyphicon-pencil'></i></button> </a></li>

  <a href='elimina_marca.php?idmar=$idmar'</a> <button class='btn btn-default btn-xs'><i class='glyphicon glyphicon-remove'></i></button> </a></li></td></tr>";
...
?>
  ...
  <form action = 'alta_marca.php'>
               <br><center><button type="submit" value="Buscar cliente" class="btn btn-primary">Registrar Nueva Marca</button></center>
                </form>
  				</div>
  			</div>
